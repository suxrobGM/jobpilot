...
3. **Save the draft** for review:
   ...
   ```bash
   curl -fsS -X PUT "$JOBPILOT_API/api/upwork/profile" -H 'content-type: application/json' \
...
1. `curl -fsS "$JOBPILOT_API/api/upwork/profile"` → require `data.status == "approved"`. If not, tell the user to review and approve on `/upwork/profile` first, then stop.
