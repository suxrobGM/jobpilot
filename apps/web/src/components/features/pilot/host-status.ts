...
/** Right after Enable there is no cycle history yet - say so instead of looking idle. */
export const PILOT_STARTING_UP_LABEL = "Starting up - first cycle begins shortly";

/** Enabled but no local host is running to execute cycles. */
export const PILOT_HOST_OFFLINE_MESSAGE =
  "Pilot is enabled but the terminal host is offline - start the JobPilot agent so cycles can run.";
