...
argument-hint: "(none â€” pulls pending URLs from /api/batch/pending)"
---
...
If the score is below `minMatchScore Ã— 10`, immediately PATCH the job to
...
- "remove 3" â€” exclude specific jobs
...
- `go` â†’ set every qualified job to `approved`
- `go 1,3,5` â†’ those become `approved`; rest become `skipped` with `skipReason: "Not selected by user"`
- `remove N` â†’ that job becomes `skipped` with `skipReason: "Removed by user"`, re-present table
- `stop` â†’ PATCH the run with `status: "paused"`, save, stop
...
Same flow as the `apply` skill â€” navigate, find Apply, follow `auth.md` if a
login page is hit.
...
Submit without per-job confirmation â€” the user approved the batch in Phase 3.
